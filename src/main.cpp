...
#define CALIBRATION_VALUE 24.70
...
unsigned long formatSensorData(unsigned int sensorValue) {
    char formattedData[11]; // 4 digits + 5 digits + null terminator
    
    // Convert sensor value to string
    char sensorStr[6]; // Maximum 5 digits + null terminator
    sprintf(sensorStr, "%05u", sensorValue);
    
    // Concatenate fixed 4-digit number and sensor value
    strcpy(formattedData, APID);
    strcat(formattedData, sensorStr);
    
    // Convert formatted string to unsigned long
    unsigned long result;
    sscanf(formattedData, "%lu", &result);
...
void setup() {
  ...
  Serial.begin(9600);
  mySwitch.enableTransmit(TXB6_Pin);
  ...
  if (newDataReady) {
    if (millis() > t + serialPrintInterval) {
      ...
      if (calculate_weight) {
        ...
        delay(200);
        unsigned long data_to_transmit = formatSensorData(actual_weight*100);
        mySwitch.send(data_to_transmit, 30);
        Serial.print("Transmitted Data: ");
        Serial.println(data_to_transmit);
...
//   Serial.println("24 bit data sent: 1111");
//   delay(1000);  
//   mySwitch.send(531010987, 30);
//   Serial.println("24 bit data sent: 531010987");
//   delay(1000);  
//   mySwitch.send(2222, 24);
//   Serial.println("24 bit data sent: 2222"); 
//   delay(2000);
// }
